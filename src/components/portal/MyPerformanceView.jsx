...
import { Download, Award, BookOpen, CheckCircle, Activity, BarChart2 } from 'lucide-react';
...
export default function MyPerformanceView({ user: _user, onNavigate }) {
  ...
  return (
    <div className="space-y-5 pb-10 text-[#172033]">
      ...
      {/* Top Header Card */}
      <div className="bg-white rounded-xl p-5 sm:p-6 border border-[#E2E8F0] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="bg-white hover:bg-[#F7F9FC] border border-[#E2E8F0] text-[#172033] font-semibold text-xs px-3.5 py-2 rounded-lg cursor-pointer transition-colors flex items-center gap-1.5 self-start sm:self-auto shrink-0"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5">
        {summaryMetrics.map((metric, idx) => (
          <div 
            key={idx}
            className="bg-white rounded-xl p-4 border border-[#E2E8F0] space-y-2.5 flex flex-col justify-between"
          >
            <div className="space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-[#64748B] block">
                {metric.label}
              </span>
              <h3 className={`text-2xl font-bold tracking-tight ${metric.isPrimary ? 'text-[#0088FF]' : 'text-[#172033]'}`}>
                {metric.value}
              </h3>
            ...
            <div className="w-full bg-[#F1F5F9] rounded-full h-1.5 overflow-hidden">
              <div 
                className={`h-full rounded-full ${metric.isPrimary ? 'bg-[#0088FF]' : 'bg-[#64748B]'}`}
      ...
      <div className="bg-white rounded-xl p-5 sm:p-6 border border-[#E2E8F0] space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#E2E8F0] pb-3">
          ...
          <span className="text-xs font-semibold text-[#172033] bg-[#F7F9FC] border border-[#E2E8F0] px-2.5 py-1 rounded-lg self-start sm:self-auto">
            Overall: 89.8%
          </span>
        </div>
          ...
          {competencyDomains.map((item, idx) => (
            <div key={idx} className="space-y-1.5">
              <div className="flex items-center justify-between text-xs">
                <span className="font-semibold text-[#172033] text-xs sm:text-sm">{item.domain}</span>
                <div className="flex items-center gap-3">
                  <span className="text-[11px] text-[#64748B]">
                    {item.status}
                  </span>
                  <span className="font-bold text-[#172033] text-xs w-8 text-right">{item.mastery}%</span>
                </div>
              </div>
      ...
      <div className="bg-white rounded-xl p-5 sm:p-6 border border-[#E2E8F0] space-y-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-[#E2E8F0] bg-[#F7F9FC] text-[11px] font-bold text-[#64748B] uppercase tracking-wider">
              ...
              {modulePerformanceData.map((m) => (
                <tr key={m.id} className="hover:bg-[#F7F9FC] transition-colors">
                  <td className="py-3 px-3">
                    <span className="font-semibold text-[#172033] block">{m.title}</span>
                    <span className="text-[10px] text-[#64748B]">{m.completedDate}</span>
                  </td>
                  ...
                  <td className="py-3 px-3">
                    ...
                    {m.status === 'In Progress' && (
                      <span className="text-[11px] font-semibold text-[#0088FF] bg-[#F0F7FF] border border-[#0088FF]/20 px-2 py-0.5 rounded">
                        In Progress
                      </span>
                    )}
                    {m.status === 'Locked' && (
                      <span className="text-[11px] text-[#64748B] bg-[#F7F9FC] border border-[#E2E8F0] px-2 py-0.5 rounded">
                        Locked
                      </span>
                    )}
                  </td>
      ...
      <div className="bg-white rounded-xl p-5 sm:p-6 border border-[#E2E8F0] space-y-4">
        ...
        <div className="space-y-2.5 pt-1">
          {assessmentHistory.map((item) => (
            <div 
              key={item.id}
              className="bg-[#F7F9FC] p-4 rounded-xl border border-[#E2E8F0] space-y-1.5"
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="space-y-0.5">
                  ...
                  <h3 className="font-bold text-[#172033] text-xs sm:text-sm">{item.title}</h3>
                </div>

                <div className="flex items-center gap-2 self-start sm:self-auto">
                  <span className="text-xs font-bold text-[#172033] bg-white border border-[#E2E8F0] px-2.5 py-1 rounded-md shadow-2xs">
              ...
              <p className="text-xs text-[#64748B] leading-relaxed pt-0.5">
                <span className="font-semibold text-[#172033] mr-1">Feedback:</span>
                {item.feedback}
              </p>
            </div>
          ))}
